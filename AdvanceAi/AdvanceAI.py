...
def advance_generate_response(user_prompt: str, context: dict[str, list[str]]):
    ...
    for i in range(len(prev_user_prompts)):
        ...
        if i < len(context["assistant"]):
            all_messages.append( {"role": "assistant", "content": prev_assistant_responses[i]} )
...
if __name__ == "__main__":
    print(advance_generate_response("Hello give me a response NOW!!!", { "user": ["Hello, I need help accessing my bank account", "What was my last question?"],
                                                                        "assistant":["You can access your account by going to the website and entering your credentials"] }))
